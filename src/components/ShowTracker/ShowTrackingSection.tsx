...
import { getTmdbWatchProviders } from '@/services/tv-shows';
...
export async function ShowTrackingSection({
  ...
  const [watchedEpisodes, tracking, viewer, watchProviders, userCountry] =
    await Promise.all([
      getWatchedEpisodes(showId),
      getShowTracking(showId),
      getViewer(),
      getTmdbWatchProviders(showId),
      getUserCountry(),
    ]);
      ...
      <div className="container-shell relative z-10 -mt-24">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:gap-8">
          ...
          <div className="flex flex-1 flex-col pb-1 md:gap-2">
            <h1 className="text-2xl font-semibold tracking-tight text-white sm:text-3xl md:mt-5">
              {details.name}
            </h1>
            ...
            {details.overview ? <ShowOverview text={details.overview} /> : null}
            <WatchProviders
...
function HomeTab({
  ...
  return (
    <div className="flex flex-col gap-5 md:gap-10">
      ...
      <section>
        <h2 className="mb-2 px-1 text-lg font-semibold text-white md:mb-4">
          Details
        </h2>
